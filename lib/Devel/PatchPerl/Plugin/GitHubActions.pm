...
sub _patch_patchlevel {
    ...
    while (<$plin>) {
        if (/\t,NULL/ and $seen) {
            print {$plout} qq{\t,"shogo82148/actions-setup-perl"\n};
        }
...
sub _patch_hints {
	# port of https://github.com/bingos/devel-patchperl/pull/69
	my $hint_base64 = 'IyMKIyBEYXJ3aW4gKE1hYyBPUykgaGludHMKIyBXaWxmcmVkbyBTYW5jaGV6IDx3c2FuY2hlekB3
...
aHJlYWRfbG9jYWw9dW5kZWYK';
	my $hint = decode_base64($hint_base64);
	my $path = File::Spec->catfile( 'hints', 'darwin.sh' );
...
sub _patch_db_file {
    my $fh;
    chmod 0666, "ext/DB_File/Makefile.PL" or die "Can't chmod ext/DB_File/Makefile.PL: $!";
    open $fh, ">", "ext/DB_File/Makefile.PL" or die "Can't open ext/DB_File/Makefile.PL: $!";
    print $fh <<'MAKEFILE';
#! perl -w
...
MAKEFILE
    close $fh or die "Can't close ext/DB_File/Makefile.PL: $!";

    open $fh, ">", "ext/DB_File/config.in" or die "Can't open ext/DB_File/config.in: $!";
    print $fh <<'CONFIG';
# Filename:	config.in
#
...
CONFIG
    close $fh or die "Can't close ext/DB_File/config.in: $!";
}
